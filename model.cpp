...
void initialize_model(const char *parameter_fname) {
  ...
  conv0_bias = buf;
  buf += 128;
  instanceNorm2d0_weight = buf;
  buf += 128;
  instanceNorm2d0_bias = buf;
  buf += 128;
  conv1_weight = buf;
  buf += 294912;
  conv1_bias = buf;
  buf += 256;
  instanceNorm2d1_weight = buf;
  buf += 256;
  instanceNorm2d1_bias = buf;
  buf += 256;
  linear1_weight = buf;
  buf += 7936;
  linear1_bias = buf;
...
// Bias : C_OUT
static void conv2d(float *in, float *out, float *weight, float *bias, int N,
                   int C_IN, int H_IN, int W_IN, int K, int C_OUT);

// MaxPool2d
// https://pytorch.org/docs/stable/generated/torch.nn.MaxPool2d.html#torch.nn.MaxPool2d
// size of in  = N * H_IN * W_IN
// size of out = N * (H / kH) * (W / kW)
static void maxpool2d(float *in, float *out, int N, int H_IN, int W_IN, int kH,
                      int kW);
...
// weight : C
// bias : C
static void instancenorm2d(float *in, float *out, float *weight, float *bias,
...
// weight : H_OUT * H_IN
// bias : H_OUT
static void linear(float *in, float *out, float *weight, float *bias, int N,
                   int H_IN, int H_OUT);
...
void model_forward(Tensor *input, Tensor *output) {
  ...
  instancenorm2d(c1->buf, i1->buf, instanceNorm2d0_weight, instanceNorm2d0_bias,
                 N, 128, 254, 254);
  ...
  instancenorm2d(c2->buf, i2->buf, instanceNorm2d1_weight, instanceNorm2d1_bias,
                 N, 256, 125, 125);
...
static void conv2d(float *in, float *out, float *weight, float *bias, int N,
                   int C_IN, int C_OUT, int H_IN, int W_IN, int K) {
  int H_OUT = H_IN - K + 1, W_OUT = W_IN - K + 1;
  for (int n = 0; n < N; n++) {
    for (int c_out = 0; c_out < C_OUT; c_out++) {
      for (int h_out = 0; h_out < H_OUT; h_out++) {
        for (int w_out = 0; w_out < W_OUT; w_out++) {
          out[n * C_OUT * H_OUT * W_OUT + c_out * H_OUT * W_OUT +
          ...
          for (int c_in = 0; c_in < C_IN; c_in++) {
            for (int kh = 0; kh < K; kh++) {
              for (int kw = 0; kw < K; kw++) {
                out[n * C_OUT * H_OUT * W_OUT + c_out * H_OUT * W_OUT +
                    h_out * W_OUT + w_out] +=
                    in[n * C_IN * H_IN * W_IN + c_in * H_IN * W_IN +
...
static void instancenorm2d(float *in, float *out, float *weight, float *bias,
  ...
  for (int n = 0; n < N; n++) {
    for (int c = 0; c < C; c++) {
      ...
      for (int h = 0; h < H; h++) {
        for (int w = 0; w < W; w++) {
          v += (in[n * C * H * W + c * H * W + h * W + w] - e) *
               (in[n * C * H * W + c * H * W + h * W + w] - e);
        }
      }
      v /= H * W;
      ...
      for (int h = 0; h < H; h++) {
        for (int w = 0; w < W; w++) {
          out[n * C * H * W + c * H * W + h * W + w] =
              (in[n * C * H * W + c * H * W + h * W + w] - e) / sqrt(v + 1e-5) *
                  weight[c] +
              bias[c];
        }
      }
